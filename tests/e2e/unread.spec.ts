...
test('unread badge appears on dm when closed', async ({ browser }) => {
  ...
  await alicePage.waitForSelector('button:has-text("Public Rooms")', { timeout: 10000 })

  // Open Bob's session
  const bobPage = await browser.newPage({ storageState: '.auth/bob.json' })
  await bobPage.goto('/')
  await bobPage.waitForSelector('button:has-text("Public Rooms")', { timeout: 10000 })
  ...
  await expect(alicePage.locator('text=HACKER_MAN')).toBeVisible()
  await expect(bobPage.locator('text=HACKER_MAN')).toBeVisible()
